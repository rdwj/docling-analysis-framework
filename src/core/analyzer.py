...
class PDFHandler(DoclingHandler):
    ...
    def _detect_pdf_subtype(self, content: str) -> str:
        ...
        if any(term in content_lower for term in ['abstract', 'introduction', 'methodology', 'references', 'bibliography']):
            return "academic_paper"
        
        # Check for report indicators
        if any(term in content_lower for term in ['executive summary', 'findings', 'recommendations', 'conclusion']):
            return "report"
        
        # Check for financial document indicators
        if any(term in content_lower for term in ['financial', 'balance sheet', 'income statement', 'cash flow']):
            return "financial"
        
        # Check for legal document indicators
        if any(term in content_lower for term in ['whereas', 'therefore', 'party of the first part', 'contract', 'agreement']):
            return "legal"
...
class GenericDoclingHandler(DoclingHandler):
    """Fallback handler for documents that Docling can process but we don't have specific handlers for"""
    ...
    def analyze_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._validate_file_size(file_path):
                file_size_mb = os.path.getsize(file_path) / (1024 * 1024)
                error_msg = f"File too large: {file_path} ({file_size_mb:.1f} MB, max: {self.max_file_size_mb} MB)"
                logger.error(error_msg)
            ...
            logger.info(f"Document analysis completed successfully in {processing_time:.2f} seconds")
            ...
            logger.error(f"Error analyzing document {file_path} after {processing_time:.2f}s: {error_details}")
